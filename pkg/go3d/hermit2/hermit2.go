...
// This is a helper sub-struct for Herm.
...
func (herm *Herm[T]) Point(t T) vec2.Vec[T] {
...
func (herm *Herm[T]) Tangent(t T) vec2.Vec[T] {
...
// Point returns a point on a hermit spline at t (0,1).
func Point[T float64 | float32](pointA, tangentA, pointB, tangentB *vec2.Vec[T], t T) vec2.Vec[T] {
	t2 := t * t
	t3 := t2 * t
...
// Tangent returns a tangent on a hermit spline at t (0,1).
func Tangent[T float64 | float32](pointA, tangentA, pointB, tangentB *vec2.Vec[T], t T) vec2.Vec[T] {
	t2 := t * t
	t3 := t2 * t
...
func Length[T float64 | float32](pointA, tangentA, pointB, tangentB *vec2.Vec[T], t T) T {
	sqrT := t * t
	t1 := sqrT * 0.5
	t2 := sqrT * t * 1.0 / 3.0
