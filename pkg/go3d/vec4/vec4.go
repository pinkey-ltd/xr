...
// Vec represents a 4 component vector.
type Vec[T float64 | float32] [4]T

// From copies a T from a generic.T implementation.
func From[T float64 | float32](other go3d.T[T]) Vec[T] {
	switch other.Size() {
	case 2:
		return Vec[T]{other.Get(0, 0), other.Get(0, 1), 0, 1}
	case 3:
		return Vec[T]{other.Get(0, 0), other.Get(0, 1), other.Get(0, 2), 1}
	case 4:
		return Vec[T]{other.Get(0, 0), other.Get(0, 1), other.Get(0, 2), other.Get(0, 3)}
	default:
...
func FromVec3[T float64 | float32](other *vec3.Vec[T]) Vec[T] {
	return Vec[T]{other[0], other[1], other[2], 1}
}
...
// Example:
//
//	vec := vec4.Vec[float64]{1, 2, 3, 4}
//	vec.Shuffle(vec4.WZYX)
//	check := vec == Vec[float64]{4, 3, 2, 1} // true
func (vec *Vec[T]) Shuffle(mask ShuffleMask) *Vec[T] {
...
//
//	vec := vec4.Vec[float64]{1, 2, 3, 4}
//	check := vec.Shuffled(vec4.WZYX) == Vec[float64]{4, 3, 2, 1} // true
func (vec *Vec[T]) Shuffled(mask ShuffleMask) (result Vec[T]) {
	result[0] = vec[mask&3]
...
func (vec *Vec[T]) Scaled(f T) Vec[T] {
	return Vec[T]{vec[0] * f, vec[1] * f, vec[2] * f, vec[3]}
}

// Invert inverts the vector.
func (vec *Vec[T]) Invert() {
	vec[0] = -vec[0]
	vec[1] = -vec[1]
	vec[2] = -vec[2]
}
...
func (vec *Vec[T]) Inverted() Vec[T] {
	return Vec[T]{-vec[0], -vec[1], -vec[2], vec[3]}
}
...
func (vec *Vec[T]) DividedByW() Vec[T] {
	oow := 1 / vec[3]
	return Vec[T]{vec[0] * oow, vec[1] * oow, vec[2] * oow, 1}
}
...
func (vec *Vec[T]) Vec3DividedByW() vec3.Vec[T] {
	oow := 1 / vec[3]
	return vec3.Vec[T]{vec[0] * oow, vec[1] * oow, vec[2] * oow}
}

// Vec3 returns a vec3.T with the first three components of the vector.
// See also Vec3DividedByW
func (vec *Vec[T]) Vec3() vec3.Vec[T] {
	return vec3.Vec[T]{vec[0], vec[1], vec[2]}
}

// AssignVec3 assigns v to the first three components and sets the fourth to 1.
func (vec *Vec[T]) AssignVec3(v *vec3.Vec[T]) *Vec[T] {
	vec[0] = v[0]
	vec[1] = v[1]
	vec[2] = v[2]
	vec[3] = 1
...
func Add[T float64 | float32](a, b *Vec[T]) Vec[T] {
	if a[3] == b[3] {
		return Vec[T]{a[0] + b[0], a[1] + b[1], a[2] + b[2], 1}
	} else {
		a3 := a.Vec3DividedByW()
		b3 := b.Vec3DividedByW()
		return Vec[T]{a3[0] + b3[0], a3[1] + b3[1], a3[2] + b3[2], 1}
	}
}
...
func Dot4[T float64 | float32](a, b *Vec[T]) T {
